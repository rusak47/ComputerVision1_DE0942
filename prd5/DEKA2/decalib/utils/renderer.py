    ...
    if type == 'pytorch3d':
        ...
        standard_rasterize_cuda = \
            load(name='standard_rasterize_cuda', 
                sources=[
			#f'{curr_dir}/rasterizer/standard_rasterize_cuda.cpp', f'{curr_dir}/rasterizer/standard_rasterize_cuda_kernel.cu'
			os.path.join(curr_dir, 'rasterizer/standard_rasterize_cuda.cpp'),
    			os.path.join(curr_dir, 'rasterizer/standard_rasterize_cuda_kernel.cu')
		], 
                extra_cuda_cflags = ['-std=c++17', '-ccbin=gcc']) # cuda10.2 is not compatible with gcc9. Specify gcc 7 
